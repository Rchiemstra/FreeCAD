...
PyObject* DocumentPy::recomputeAsync(PyObject* args)
{
    ...
    PY_TRY
    {
        ...
        RecomputeTargets targets;
        for (auto* obj : objs) {
            if (obj) {
                targets.objectIds.push_back(obj->getID());
            }
        }
        targets.forceAll = Base::asBoolean(force);

        RecomputeOptions options;
        options.allowAsync = true;
        options.maxGuiSlice = std::chrono::milliseconds(33);

        RecomputeHandle handle = getDocumentPtr()->getRecomputeCoordinator().request(targets, options);
        return Py::new_reference_to(Py::Long(handle.id()));
    }
    PY_CATCH;
}
