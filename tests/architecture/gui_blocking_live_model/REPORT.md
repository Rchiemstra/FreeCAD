...
This report summarizes the reproducible inventory (1,806 findings across seven
...
| --- | ---: | --- | --- |
| `live-app-dereference` | 1,034 | investigate | `App::GetApplication().getActiveDocument()/getDocument()/getDocuments()` and `FreeCAD.ActiveDocument` reach. |
| `live-reference-callback` | 206 | migrate | State-change signal/slot callbacks and observer subscriptions carrying live references. |
| `update-data-provider` | 187 | migrate | `ViewProvider::updateData` overrides (qualified and inline). |
| `direct-recompute` | 309 | migrate | Synchronous `recompute()` from GUI commands/tasks/dialogs. |
| `process-events-polling` | 45 | investigate | Manual event-loop pumping. |
...
The snapshot spans 751 C++ and 1,055 Python findings. Owning subsystems:
`BIM` (568), `CAM` (349), `Gui` (324), `Draft` (128), `Part` (87), `Fem`
(74), `Mesh` (62), `PartDesign` (46), `TechDraw` (41), `Surface` (21),
`Measure` (18), `Assembly` (17), and smaller workbenches (22 subsystems in
total).
...
* The scanner covers `src/Gui`, `src/Mod/*/Gui`, production workbench
  ...
  the README. Unreviewed App/test/import helpers and the `src/App` model core
  remain out of scope.
