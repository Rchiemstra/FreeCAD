...
**Current snapshot:** Phases 1–3 are complete. Phase 3 delivered bounded detached preparation beside the existing recompute worker, serialized live commit/recompute admission, lifetime-safe close and callback handling, identity-bound queued recomputes, and detached Part Boolean preparation with copy-isolated retries and a bounded semantic shape digest. Focused and full Docker gates and all critical reviews passed. Phase 4 is next. The MCP submodule was untouched at its existing gitlink and untracked `tests/lib/` remains excluded. Phase 7 is unblocked but remains ordered after Phases 1–6.
