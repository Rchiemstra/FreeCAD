...
when its exit criterion is demonstrated. P2-P9 are landed; P10 is the only
code-review follow-up identified in the 2026-07-31 post-landing review.
...
| P10 | Distinguish a healthy long call from an actual stale recovery | Not started | S4 + coordinator | A tool that exceeds the stale threshold while heartbeats remain healthy produces no false `recovered` outcome; any idempotent reconcile is classified as a no-op, while a real watchdog race still reports and performs recovery. |
...
**P9 closeout work (complete, 2026-07-31):**
...
The restart-free recovery path works, but post-tool observability currently
cannot distinguish “the watchdog made this lease stale and it was recovered”
from “heartbeats kept this lease healthy for the entire long call.”

The false-positive path is deterministic:

1. `server._post_tool_stale_recovery` treats elapsed time alone as evidence and
   passes every held document to
   `StaleLeaseRecoveryOrchestrator.observe_tool_completion` after the threshold.
2. The orchestrator marks all those sessions as needing recovery and calls
   `lease_reconcile`, even when the heartbeat loop kept the lease
   `LOCKED_IDLE`.
3. `_stale_reconcile_already_recovered` deliberately accepts that
   `LOCKED_IDLE` lease as an idempotent success, but `lease_reconcile` removes
   the `idempotent` marker when returning from its prepare phase.
4. `_recover_one_locked` classifies every successful response as
   `outcome: recovered`. The health record therefore claims a recovery that did
   not happen and the normal long-call path performs an unnecessary control
   RPC.

A focused reproduction against the landed orchestrator supplied
`success=true`, `idempotent=true`, and `lease.state=LOCKED_IDLE`; the emitted
status was still `outcome=recovered`, `attempted=true`, and `succeeded=true`.
This does not invalidate the P8 restart-free recovery pass, but it weakens P6
diagnostics and makes future incidents harder to distinguish from healthy long
operations.

P10 requires:

1. Preserve an explicit `idempotent` or `already_active` result from the
   FreeCAD RPC layer through `FreeCADClient.reconcile_document_lease`.
2. Have the orchestrator classify that result as no recovery needed, not
   `recovered`. Prefer avoiding the reconcile call entirely when a fresh
   heartbeat result already proves the lease remained active.
3. Do not overwrite the last real recovery evidence with a later
   healthy/no-op post-tool check.
4. Add a regression test with a synchronous call longer than the accelerated
   stale threshold while heartbeats remain successful. Assert that the lease
   stays `LOCKED_IDLE`, recovery success is not reported, and the real stale
   watchdog-race test still reports `recovered`.
5. Re-run the focused orchestration/status tests and the Docker
   `unit`, `e2e`, `core`, and `benchmark` suites before marking P10 complete.

**Independent review evidence (2026-07-31):** the submodule is clean at
`c6f3c4fef822`. Two local focused runs passed 139 tests covering worker lanes,
orchestration, dirty reconcile, stale races, identity refresh, selector
isolation, diagnostics, status, GUI guidance, and worker-boundary regressions.
The first invocation without `PYTHONPATH=src` failed during collection because
the package was not importable in that shell; rerunning with the repository's
source path produced the passing results above and is not a product failure.
...
| Wave | Work | Maximum worker subagents | Status | Finish condition |
| --- | --- | --- | --- | --- |
| M0 | Independent source/design verification by S1-S7 | **7 (MAX)** | Complete | Findings reconciled into D1-D9. |
| M1 | Isolated implementation by S1-S6 plus continuous read-only review by S7 | **7 (MAX)** | Complete | Scoped implementation landed without shared production-file ownership. |
| M2 | Parallel verification: runtime, identity, reconcile, orchestration, handoff regression, static checks, and security review | **7 (MAX)** | Complete | Integrated test and review reports were reconciled before P8. |
...
| M6 | Implement and independently verify P10 | **2** | **Next** | One S4 implementer and one read-only reviewer demonstrate accurate healthy-long-call and real-stale outcomes, then the coordinator reruns all Docker suites and marks P10 complete. |
...
the orchestration/status contract, so additional editing agents would increase
merge risk without adding an independent workstream. M4 intentionally used one
agent because multiple agents must not control the same FreeCAD process,
document, lease, or sidecar concurrently.
