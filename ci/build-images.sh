...
# (default 192.168.2.23:5001 -- the dedicated FreeCAD registry API published on the LAN).
#
...
# agent (where registry-freecad:5000 resolves). Use this script only if you want to build
# from your own machine -- which requires your Docker daemon to reach 192.168.2.23:5001
# AND to list it under insecure-registries (it is an HTTP registry):
#   Settings -> Docker Engine -> {"insecure-registries": ["192.168.2.23:5001"]}
#
...
#
# Overrides (env):
#   REGISTRY=192.168.2.23:5001   # registry host:port
#   NS=                      # optional namespace segment (default none -> registry/<repo>:tag)
set -euo pipefail

REGISTRY="${REGISTRY:-192.168.2.23:5001}"
