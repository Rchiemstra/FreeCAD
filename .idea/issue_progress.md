...
- Pipeline **363** failed `freecad-e2e` on `d8c31b4bf7` (exit 1). Aggregate `TestSketcherGui` **52** tests (recovery test added). Distance-datum hover was no longer the failure. `test_chain_link_offsets_connected_open_edges_from_single_selected_edge` did not add offset geometry; the class-only `TestSketcherOffsetGui` **6 tests OK**. Classification: the new Inf-camera recovery test runs last in `SketcherGuiTestCases` and can leave an Inf ortho height on the shared 3D view for later classes. Restore a finite height in that test's `finally` and in `tearDown`.
- Pipeline **364** failed `freecad-e2e` on `3bb1c46363` (exit 1). `test_inf_camera_recovers_finite_pick_and_edge_preselection` **ok** (aggregate and class unit). Offset chain-link **ok**. Aggregate `FAILED (failures=5)` including `test_origin_marker_tracks_drawing_tool_state`. Class-only `TestConstraintCommandsGui` `FAILED (failures=3)` (`ConstraintCount` 0 != 1 on continuous picking). Those command tests are not in the Inf-camera module; 363's class-only ConstraintCommands run had passed. Do **not** merge `3bb1c46363`.
- Do **not** merge until `ci/woodpecker/pr/ci` is **success** for the **exact pushed SHA**.
